...
setMiners = function (miners) {
  ...
  for (const i in sortedMiners) {
    const miner = miners[i];
    const identifier = miner.identifier;

    if (!minerStats[identifier]) {
      minerStats[identifier] = {
        threads: 0,
        hashrate: 0,
        accepted: 0,
        rejected: 0,
    ...
    // Accumulate stats
    minerStats[identifier].threads += 1;
    minerStats[identifier].hashrate += miner.hashrate;
    minerStats[identifier].accepted += miner.accepted;
    minerStats[identifier].rejected += miner.rejected;
  }

  for (const identifier in minerStats) {
    num++;
    const miner = minerStats[identifier];
    ...
    const ping =
      miner.ping > 300
        ...
        : `<small>${timeFormatted(miner.ping)}</small>`;
